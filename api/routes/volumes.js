...
router.get('/', (req, res, next) => {
    ...
    const volumes = Volume
        ...
        .skip(offset)
        .limit(limit)
        .sort({ created_at: 1 }).then(volumes => {
...
router.post('/', (req, res, next) => {
    if (utils.requestIsEmpty(req.body))  // Check if request body is empty
        res.status(400).json({ message: 'Cannot create volume, empty request.' }); // If enmpty status 400 and send message
    else{
...
router.delete('/:id', (req, res, next) => {
    if (utils.requestIsEmpty(req.params.id) || !ObjectId.isValid(req.params.id)) // Check if id is empty, status 400 and message
      ...
      const volume = Volume
          .findOne({ // Find one volume by id
              _id: req.params.id
          })
          .then(volume => {
              Volume.deleteOne({ // Delete one volume
                  _id: req.params.id
              }).then(result => {
