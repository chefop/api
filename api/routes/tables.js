...
router.post('/', (req, res, next) => {
    if (utils.requestIsEmpty(req.body))  // Check if request body is empty
        res.status(400).json({ message: 'Cannot create table, empty request.' }); // If enmpty status 400 and send message
    else{
...
router.delete('/:id', (req, res, next) => {
    if (utils.requestIsEmpty(req.params.id) || !ObjectId.isValid(req.params.id)) // Check if id is empty, status 400 and message
      ...
      const table = Table
          ...
          .then(table => {
              Table.deleteOne({ // Delete one table
                  _id: req.params.id
              }).then(result => {
